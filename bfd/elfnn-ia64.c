			 ...
			 const Elf_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < relend; ++rel)
	...
	h = NULL;
      ...
      if (h && UNDEFINED_WEAK_RESOLVED_TO_ZERO (info, h))
	continue;
  ...
  if (dyn_i->want_fptr)
	...
	while (h->root.type == bfd_link_hash_indirect
	  ...
	  && (!h
		  ...
		  && !UNDEFINED_WEAK_RESOLVED_TO_ZERO (x->info, h))
	      || (h->root.type != bfd_link_hash_undefweak
			 ...
			 void * data)
{
  ...
  resolved_zero = (dyn_i->h
		   && UNDEFINED_WEAK_RESOLVED_TO_ZERO (x->info,
						       dyn_i->h));
  ...
  if (! done)
    {
      ...
      if (((bfd_link_pic (info)
		...
		|| (ELF_ST_VISIBILITY (dyn_i->h->other) == STV_DEFAULT
		    && !UNDEFINED_WEAK_RESOLVED_TO_ZERO (info, dyn_i->h))
		|| dyn_i->h->root.type != bfd_link_hash_undefweak)
  ...
  for (; rel < relend; ++rel)
	...
	case R_IA64_DIR64LSB:
	  ...
	  if ((dynamic_symbol_p || bfd_link_pic (info))
	      && !(h && UNDEFINED_WEAK_RESOLVED_TO_ZERO (info, h))
	      && r_symndx != STN_UNDEF
