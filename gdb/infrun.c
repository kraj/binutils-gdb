  ...
  for (thread_info *tp = threads_to_step; tp != NULL; tp = next)
    {
	 ...
	 in the step over chain indefinitely if something go wrong when resuming
	 it.  If the error is intermittent and it still needs a step over, it
	 will get enqueued again when we try to resume it normally.  */
