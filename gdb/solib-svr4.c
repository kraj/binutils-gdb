...
static void svr4_free_library_list (struct so_list *solist);
...
struct svr4_info
{
  ...
  std::map<CORE_ADDR, struct so_list *> solib_lists;
};
...
static bool
svr4_is_default_namespace (const struct svr4_info *info,
...
static void
free_solib_lists (struct svr4_info *info)
{
  for (const std::pair<CORE_ADDR, struct so_list *> tuple
	 : info->solib_lists)
...
static void
svr4_free_library_list (struct so_list *list)
{
  while (list != NULL)
    {
			      ...
			      std::vector<gdb_xml_value> &attributes)
{
  ...
  struct so_list **solist = &list->solib_lists[0];
  *solist = nullptr;
  list->tailp = solist;
}
...
svr4_current_sos_direct (struct svr4_info *info)
{
  ...
  for (; debug_base != 0;
    ...
    {
      /* Walk the inferior's link map list, and build our list of `struct
	 so_list' nodes.  */
      lm = solib_svr4_r_map (debug_base);
      if (lm != 0)
	{
	  struct so_list **sos = &info->solib_lists[debug_base];
	  *sos = nullptr;
...
{
  struct so_list *sos = nullptr;
  struct so_list **pnext = &sos;

  for (const std::pair<CORE_ADDR, struct so_list *> tuple
	 : info->solib_lists)
    {
      struct so_list *solist = tuple.second;
			   ...
			   CORE_ADDR lm)
{
  ...
  struct so_list **psolist = &info->solib_lists[debug_base];
  struct so_list **pnext = nullptr;
  struct so_list *solist = *psolist;
  CORE_ADDR prev_lm;
...
static CORE_ADDR
find_debug_base_for_solib (struct so_list *solib)
{
  if (solib == nullptr)
    return 0;
   ...
   objfile *current_objfile)
{
  ...
  /* The linker namespace to iterate identified by the address of it's
