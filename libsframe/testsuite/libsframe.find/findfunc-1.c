...
/* sframe_get_funcdesc_with_addr is a static API in the libsframe library, but
   it is another core API which supports information lookup via
   sframe_find_fre.  The latter is user-interfacing and is the mainstay for an
