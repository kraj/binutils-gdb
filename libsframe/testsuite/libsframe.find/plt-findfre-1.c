...
static int
add_plt_fde1 (sframe_encoder_ctx *encode, int idx)
{
  int i, err;
  /* A contiguous block containing 3 FREs.  */
  ...
  err = sframe_encoder_add_funcdesc (encode, 0x1000, 16*5, finfo, 3);
  if (err == -1)
    return err;
