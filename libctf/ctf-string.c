...
/* Remove all movable refs to a given atom.  */
static void
ctf_str_purge_movable_atom_refs (ctf_str_atom_t *atom)
{
...
ctf_str_free_atom (void *a)
{
  ...
  ctf_str_purge_movable_atom_refs (atom);
...
void
ctf_str_purge_refs (ctf_dict_t *fp, int movable)
{
  ctf_dynhash_iter (fp->ctf_str_atoms, ctf_str_purge_one_atom_refs, &movable);
}
...
ctf_str_write_strtab (ctf_dict_t *fp)
{
  ...
  ctf_str_purge_refs (fp, 0);
  ctf_str_purge_refs (fp, 1);
